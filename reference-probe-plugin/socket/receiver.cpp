...
std::string extractXMLElement(IXMLDOMDocument* doc, const std::wstring& tag) {
    ...
    BSTR value;
    node->get_text(&value);
    std::string result = (const char*)_bstr_t(value);
    SysFreeString(value);
    node->Release();
    return result;
}
    ...
    while (true) {
        ...
        if (bytesReceived > 0) {
            ...
            if (success == VARIANT_TRUE) {//load check and subsequent variable load
                request_id = extractXMLElement(doc, L"/request/request_id");
                payload = extractXMLElement(doc, L"/request/payload");
                responsePayload = payload + "----response";

                //////////////////////////////////////////////////////////////////////////    
                ////////////////////////////RESPONSE//////////////////////////////////////
                std::string xmlResponse = buildXMLResponse(request_id, responsePayload);
                send(clientSocket, xmlResponse.c_str(), (int)xmlResponse.size(), 0);

...
//build cmd: cl transmitterInit.cpp /Fe:transmitterInit.exe /EHsc ws2_32.lib
...
std::string buildXMLRequestInit(const std::string& request_id) {
    std::ostringstream oss;
    std::string initialize = "True";
    std::vector<std::uint8_t> initializeVector = {0x00};
    std::string initValue = bytes_to_hex_list(initializeVector);
    size_t bitSize = 0;
    oss << "<request>"
...
int main() {
    ...
    std::string xml = buildXMLRequestInit(request_id);
